...
handle_call({transaction, List, CRDTActor}, _From,
    ...
    Result = case lasp_config:get(intermediary_node_modification,
                                  ?INTERMEDIARY_NODE_MODIFICATION) of
        true ->
            Result0 = ?CORE:transaction(List, CRDTActor, ?CLOCK_INCR(Actor), ?CLOCK_INIT(Actor), Store),
            Final = {ok, {_, _, Metadata, _}} = declare_if_not_found(Result0, List, State, ?CORE, transaction,
                                 [List, Actor, ?CLOCK_INCR(Actor), Store]),
            case lasp_config:get(blocking_sync, false) of
                true ->
                    ok = blocking_sync(List, Metadata);
                false ->
                    ok
            end,
            Final;
        false ->
            {ok, IsRoot} = case ?DAG_ENABLED of
                               true ->
                                   lasp_dependence_dag:is_root(List);
                               false ->
                                   {ok, true}
                           end,
            case IsRoot of
                false ->
                    {error, {intermediary_not_modification_prohibited, List}};
                true ->
                    Result0 = ?CORE:transaction(List, CRDTActor, ?CLOCK_INCR(Actor), ?CLOCK_INIT(Actor), Store),
                    Final = {ok, {_, _, Metadata, _}} = declare_if_not_found(Result0, List, State, ?CORE, transaction,
                                         [List, Actor, ?CLOCK_INCR(Actor), Store]),
